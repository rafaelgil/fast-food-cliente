...
class ClienteStepDefinition {
    ...
    fun `submeter um novo cliente`(): ClienteResponse {
        val clienteRequest: Any = clienteRequest("12345678901", "João da Silva", "joao@gmail.com")
        response = given()
    ...
    fun `submeter um novo cliente com email inválido`() {
        val clienteRequest: ClienteRequest = clienteRequest("12345678902", "Maria Oliveira", "maria_email_invalido")
        response = given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(clienteRequest)
                .`when`().post(ENDPOINT)
    ...
    fun `que existe um cliente cadastrado com o CPF`(cpf: String) {
        val clienteRequest: ClienteRequest = clienteRequest(cpf, "João da Silva", "joao_cpf_invalido@gmail.com")
        given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(clienteRequest)
                .`when`().post(ENDPOINT)
    ...
    fun `submeter um novo cliente com o mesmo CPF`(cpf: String) {
        val clienteRequest: ClienteRequest = clienteRequest(cpf, "Maria Oliveira", "maria_cpf_invalido@gmail.com")
        response = given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(clienteRequest)
                .`when`().post(ENDPOINT)
    ...
    fun `que existe um cliente cadastrado com o email`(email: String) {
        val clienteRequest: ClienteRequest = clienteRequest("12345678904", "João da Silva", email)
        given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(clienteRequest)
                .`when`().post(ENDPOINT)
    ...
    fun `submeter um novo cliente com o mesmo email`(email: String) {
        val clienteRequest: ClienteRequest = clienteRequest("78901234567", "Maria Oliveira", email)
        response = given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(clienteRequest)
                .`when`().post(ENDPOINT)
    ...
    fun `que existe um cliente cadastrado com o CPF`(cpf: String, email: String) {
        val clienteRequest: ClienteRequest = clienteRequest(cpf, "Cliente Existente", email)
        given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .body(clienteRequest)
                .`when`().post(ENDPOINT)
