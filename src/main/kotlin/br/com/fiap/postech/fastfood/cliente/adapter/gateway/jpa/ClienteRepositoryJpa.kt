...
interface ClienteRepositoryJpa : JpaRepository<ClienteSchema, UUID> {
    ...
    @Query("SELECT c FROM cliente c WHERE c.nome = :nome AND c.cpf = :cpf AND c.email = :email AND c.endereco = :endereco AND c.telefone = :telefone AND c.excluidoEm IS NULL")
    fun buscarClientePorNomeCpfEmail(@Param("nome") nome: String, @Param("cpf") cpf: String, @Param("email") email: String, @Param("endereco") endereco: String, @Param("telefone") telefone: String): ClienteSchema
}